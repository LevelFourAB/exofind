#!/usr/bin/env bash
#
# Load the Discogs example into a node: define the index, put every record in
# it and commit, which is what makes them searchable and pushes them to
# storage.
#
...
# batches rather than as one post of every record there is. A record carries
# its pressings, so it is several times the size of a document in the other
# examples and the batches are correspondingly smaller.
BATCH="${BATCH:-1500}"
