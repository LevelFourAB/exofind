...
- **Deleting an index:** `Indexes.delete` removes the registry entry and the local copies, then leaves a removal mark in the bucket through `IndexRemovals`. `IndexRemovalSweeper` removes marked prefixes on indexer candidates once the mark is older than `exofind.indexes.removal.grace`, and `Indexes.create` clears a marked prefix before opening a generation on it. `ObjectStorageRegistryAudit` reports marks and skips marked storage unless a repair restores it.
- **Facet state per reader:** `FacetStates` keeps what counting a facet needs per reader and per segment: ordinal maps, columns, and the counts over everything a segment holds. `FacetWarmer` queues an index after every reopen and builds that state for every faceted field before the first search asks. `exofind.search.warm-threads` sizes the warm threads, and `0` turns warming off.
- **Searching the values of a facet:** `SearchResource.facetValues` answers the values of one facet field that start with a prefix. It runs a search with a limit of zero and one `Facet` carrying the `prefix`, so the counts and the scope cache are the ones a search uses. `StringFacetCount` picks the values off `FoldedTerms`, a per-segment dictionary folded by the field's autocomplete normalize step and kept in `FacetStates`.
- **Declared values of a facet field:** The search settings can declare values of a string facet field with an `order` and `labels` per locale. `DeclaredValues` compiles them per generation and resolves the labels of a search locale once per locale. `StringFacetCount` answers `Facet.Order.DECLARED`, carries the label on each `SearchResult.Facet.Value`, and matches a prefix against the labels. `ValueReader` reads a typed label as its value. The declaration is named `declared_values` in `SearchSettingsFeatures`.
...
- **Reading a search box:** `UserText` splits typed text into words, phrases and exclusions. `Index.search` then runs `Interpretation` before it compiles: `QuantityReader` reads numbers next to the unit spellings `Units` derives from a field's declared `unit`, or next to the comparative words a `LocaleSupport` lists, and turns them into filters reported as `SearchResult.Interpreted`. A text clause may name the `TextQuery.Target` list a reading is allowed on, each with the clauses that pin it to one value of a nested list and the targets read instead where a document holds nothing. `ValueReader` then looks the words that are left up among the values of the string facet fields the search settings opt in through `ValueDictionaries`, using the `FoldedTerms` a facet prefix search folds with. `Relaxation` drops words only from the text that is left.
...
- **Feature registration in `IndexFeatures`, `AuthFeatures` and `SearchSettingsFeatures`:** Register any new usage, type, or capability in `IndexFeatures`. Register any key constraint in `AuthFeatures`. Register any new part of the search settings in `SearchSettingsFeatures`. Protocol Buffers preserves unknown fields during decoding. Without an explicit feature name, an older node reads a newer definition, ignores unrecognized fields, and indexes data incorrectly, or searches with part of the settings.
...
- **What says that stored data was deleted:** Only a removal mark under a prefix does, see `IndexRemovals`. A prefix the registry does not name looks the same whether the index was deleted or the registry was lost, and the registry repair rebuilds a lost registry from exactly those prefixes. A sweep or a creation that removes objects from registry absence alone destroys data a repair would have brought back, and no test can tell the two apart.
...
- **What keeps norms dense:** Every Lucene document written to an index carries an entry for every analyzed field of the schema, empty where the document has no text, so Lucene stores the norms scoring reads as an array instead of a sparse structure. `AnalyzedFields` enumerates those fields by asking each type through `FieldType.collectAnalyzedFields`, and `Index.addDocument` fills out the document and the values of its object fields. An analyzed field a type writes but does not report, or a new path that writes Lucene documents without padding them, sends scoring back through sparse norms with no test failure and no change in results.
- **Which threads a search may borrow:** Only the ones `SearchThreads` lends, through `invokeAll` or the executor it hands to `IndexSearcher`. Those carry the `SearchDeadline` budget of the request thread; a piece of search work run on any other executor collects without a budget, and nothing reports that it ran past the timeout.
