...
| `exofind.commit` | Timer | Seconds | `trigger` (`changes`, `interval`, `merges`, `explicit`, `freshness`), `outcome` (`success`, `error`) | Duration of a Lucene commit operation. A `freshness` trigger is a commit that a search waiting on a freshness token asked the writer for. |
...
The `state` tag carries the uppercase name of the state. The admin API answers with the lowercase form of the same name, such as `needs_pull`. See [Index states](admin-api.md#index-states).
