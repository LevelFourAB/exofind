...
The code says what the caller did wrong, not which part of the engine found it. A search that names a field the index does not have reports `search:field_unknown` whether the mapper or the index answered, and a change to some of a document reports `document:patch:*` where the same mistake in a change to some of the search settings reports `settings:patch:*`. The two families hold the same last segments and differ only in the prefix. For the whole list, see [Change paths](patch-paths.md).
...
- `search:clauses_too_many`: Returned when a request holds more clauses than `EXOFIND_SEARCH_MAX_CLAUSES`, counted across `query`, `filters`, `hits.when`, `rescore.boost`, the `when` of every interpret target of a `text` clause, and the `when` of every sort, facet, and fallback target. The `path` names the clause the count ran past. The rest of the request is not validated, so a client that also has other errors to fix sees them only after this one.
- `search:clauses_too_deep`: Returned when clauses nest deeper than `EXOFIND_SEARCH_MAX_CLAUSE_DEPTH`. Flatten the query: `and` inside `and` narrows the same way as one `and` holding both clauses. Each `fallback` of an interpret target, a sort, or a facet counts as one level, as does the `when` of a target.
...
- `search:sort:type_unsupported`: Returned when a `field` sort with `when` or `fallback` names a field that is not a number or timestamp field. Sort by the field without `when` and `fallback`, or name a number or timestamp field. See [Sorting by the value a customer sees](search-api.md#sorting-by-the-value-a-customer-sees).
- `search:sort:fallback_type_mismatch`: Returned when a `fallback` of a `field` sort names a field of another type than the field of the sort. The `name` and `type` arguments carry the fallback, and `field` and `expected` carry the field of the sort. Every field of the chain must be of one type, so the values of two documents compare.
- `search:facet:fallback_unsupported`: Returned when a facet carries `when` or `fallback` and no `ranges`. The `field` argument carries the field of the facet. Add `ranges`, or remove `when` and `fallback` to count every value. See [Counting by the value a customer sees](search-api.md#counting-by-the-value-a-customer-sees).
- `search:facet:fallback_type_mismatch`: Returned when a `fallback` of a facet names a field of another type than the field of the facet. The `name` and `type` arguments carry the fallback, and `field` and `expected` carry the field of the facet.
- `search:hits:sort_fallback_unsupported`: Returned when a search whose hits are the values of a nested list is sorted with `when` or `fallback`. Each hit is ordered by its own value, so remove `when` and `fallback` from the sort.
...
- `settings:patch:value_invalid`: Returned when a `PATCH` of search settings names a field that cannot hold the given value, such as a list where the settings hold an object. The codes for a path a change cannot use are listed in [Change paths](patch-paths.md).
...
- `document:key_conflicting`: Returned when the body of a `PUT` or `PATCH` of one document gives the primary key field a value other than the key in the URL. The index definition says which field is the primary key, and the URL names the document.
...
- `request:body_too_large`: Returned with HTTP `413` when the request body is larger than the node accepts. The `limit` argument carries the size in bytes, and the response closes the connection. A node states one size for a body it holds in memory and another for a newline-delimited body it reads as it arrives, neither of which a client can read from the API; see [Configuration](configuration.md#api-requests). A newline-delimited request is indexed as it is read, so its response also carries `processed`, how many documents the index took before the body was cut off - send the ones after that rather than the whole dataset again. A body in any other media type is rejected before any of it is read, so nothing was written.
