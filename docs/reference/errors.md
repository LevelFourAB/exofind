...
- `search:suggest:limit_invalid`: Returned with HTTP `400` by `POST /v1alpha1/indexes/{name}/suggest` when `limit` is outside 1 to 100. The `max` argument carries the largest limit allowed.
