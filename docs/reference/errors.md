...
- `request:update:no_match`: Returned when a partial document update names one value of a field by a selector, such as `variants[sku=V-2]`, and the document holds no value the selector matches. A selector never creates the value it names, so add a value with `variants[]` instead. The remaining `request:update:*` codes report a path the endpoint cannot use and are listed in the [Documents API](documents-api.md#constraints-and-errors).
