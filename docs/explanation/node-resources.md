...
- **Facet warming off the request path:** The first facet count against a reopened reader must build the ordinal maps and the whole-index counts of that reader, and that work costs the same however few documents the search matched, so without warming the first search after every pull or commit pays for it. When a pull or commit reopens an index reader, warming runs on dedicated threads configured by `EXOFIND_SEARCH_WARM_THREADS` from a queue that holds each index once. A search that arrives first builds what it needs itself. A search that arrives while warming builds the ordinal map for its field waits for that build instead of starting a second one. The `exofind.facet.warm` meter times each warm operation, and `exofind.facet.warm.queued` reports how many indexes are waiting.
